...
ADD money DOUBLE(10,2) AFTER emp_name; # 添加到第一列
```
...
## 24.4. MyISAM中的索引方案
MyISAM InnoDB Memory都支持B-树索引。但Innodb和MyISAM默认的索
...
## 35.2. 快照读与当前读
MVCC在MySQL InnoDB中的实现主要是为了提高数据库并发性能，用更好的方式去处理读-写冲突，做到即使有读写冲突时，也能做到 不加锁 ，非阻塞并发读 ，而这个读指的就是 快照读 , 而非 当前读 。当前
...
- redo log 它是 物理日志 ，记录内容是“在某个数据页上做了什么修改”，属于 InnoDB 存储引擎层产生的。为事务的持久性服务的。
